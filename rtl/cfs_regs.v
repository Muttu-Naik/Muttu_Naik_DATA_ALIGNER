...
//    parameter STATUS_RX_LVL_WIDTH   = 8,
//    parameter STATUS_TX_LVL_WIDTH   = 8,
...
assign irq = 
      ...
      (edge_max_drop      & irqen_max_drop);  //********added
  ...
  always @(posedge pclk or negedge presetn) begin
    if (presetn == 0) begin
      ...
      if (psel == 1 && penable == 1) begin   //*******if (psel == 0 && penable == 0)
        if (pready == 0) begin
          case (addr_aligned)
            ADDR_CTRL: begin
              if (pwrite) begin
                if (wr_ctrl_is_illegal == 0) begin
                  if (ctrl_size_wr_val == 0) begin
                    ...
                    ctrl_clr           <= pwdata[LSB_CTRL_CLR]; //****** ~pwdata[LSB_CTRL_CLR];
                  end
                end else begin
                  wr_ctrl_is_illegal <= 0;
                  pready             <= 1;
