...
module cfs_synch_fifo #(
  ...
  always_comb begin
//    push_fifo_lvl = 0;  //default value so that i wont be Z
    if (wr_ptr_push == rd_ptr_push) begin
      if (push_fifo_lvl_dly >= FIFO_DEPTH - 1) begin
        push_fifo_lvl = FIFO_DEPTH;     //status tx level
      end else begin
        push_fifo_lvl = 0;
      end
  ...
  always_comb begin
//    pop_fifo_lvl = 0;  // default value so that it wont be Z
    if (wr_ptr_pop == rd_ptr_pop) begin
      if (pop_fifo_lvl_dly >= FIFO_DEPTH - 1) begin
        pop_fifo_lvl = FIFO_DEPTH;   //status rx level
      end else begin
        pop_fifo_lvl = 0;
      end
  ...
  assign pop_valid = !pop_empty;  //correct
