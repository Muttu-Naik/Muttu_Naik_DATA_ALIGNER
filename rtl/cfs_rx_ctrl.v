...
if (md_rx_valid & md_rx_ready & md_rx_err)
//----------- if (md_rx_valid & push_ready & md_rx_err)
begin
...
/*  always_comb begin
  if (md_rx_valid && md_rx_ready) begin
...
*/
always_comb begin
  md_rx_err = 1'b0;   // default

  if (md_rx_valid && md_rx_ready) begin
//-----  if (md_rx_valid) begin
    if (md_rx_size == 0) //******** (md_rx_size == 0) &&  (md_rx_size == 1)
      md_rx_err = 1'b1;
    else if ((((ALGN_DATA_WIDTH / 8) + md_rx_offset) % md_rx_size) != 0)
      md_rx_err = 1'b1;  //********* md_rx_err = 1'b0;
  end
end

  assign md_rx_ready = (push_valid & push_ready) | md_rx_err;
//---------- assign md_rx_ready = push_ready;

  assign push_valid = md_rx_valid & !md_rx_err;
//----------assign push_valid = md_rx_valid & !md_rx_err & !rx_fifo_full;
